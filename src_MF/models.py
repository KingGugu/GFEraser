...
class PreFilter(nn.Module):
    ...
    def predict(self, u):
        # filtered_user_emb = self.user_embedding.weight - self.neg_user_embedding.weight
        # filtered_item_emb = self.item_embedding.weight - self.neg_item_embedding.weight
...
class Original(nn.Module):
    ...
    def predict(self, u):
        u = [int(i) for i in u]
        score = torch.matmul( self.user_embedding.weight[u], self.item_embedding.weight.transpose(0, 1))
        return score.detach().cpu().numpy()
